...
_HPC_PARTITION  = os.getenv("HPC_PARTITION", "gpu")            # Anvil NAIRR allocation
_HPC_QOS        = os.getenv("HPC_QOS", "gpu")                 # Anvil requires this
...
def _slurm_script(pdb_id: str, remote_dir: str) -> str:
    ...
    gpu_line     = "#SBATCH --gpus-per-node=1\n"         if "gpu" in _HPC_PARTITION.lower() else ""
    return f"""\
#!/bin/bash
